...
Trois stacks progressives, autonomes : `init → plan → apply → destroy`.
...
> Le serveur NFS du TP 14 est **votre poste Ubuntu**, configuré par Ansible
> (`lab/ansible/nfs-local.yml`) : pas de stack Terraform pour lui.
...
Le token doit avoir le rôle `TerraformProv` (TP 06 §7), et `ssh root@<votre-noeud>`
doit fonctionner **sans mot de passe** : le provider dépose les snippets cloud-init par
SCP.
...
`/etc/pve/firewall/cluster.fw` en **ressources natives** du provider
...
privé, EVPN compris) qui rendent les VM joignables depuis le poste.
